...
public class SingularExtension extends CheckersSearcher {
    ...
    public SingularExtension(ToIntFunction<Checkerboard> e) {
        super(e);
    }
    ...
    public Optional<Duple<Integer, Move>> selectMove(Checkerboard board) {
        return selectMoveHelper(0, board, 0, alpha, beta);
    }

    public Optional<Duple<Integer,Move>> selectMoveHelper(int dmaxModifier, Checkerboard board, int depth, int recursiveAlpha, int recursiveBeta) {
        ...
        for (Checkerboard futureBoard :
            ...
            if (futureBoard.getCurrentPlayer() == adversary) {
                recursiveResult = selectMoveHelper(0,futureBoard, depth + 1, -recursiveBeta, -recursiveAlpha);
                ...
                recursiveResult = selectMoveHelper(0,futureBoard, depth + 1, recursiveAlpha, recursiveBeta);
                if (recursiveResult.isPresent()) {
                    totalEval+=recursiveResult.get().getFirst();
                    usedBoardsEval.put(futureBoard,recursiveResult.get().getFirst());
        ...
        for (Checkerboard used : moves) {
                if (usedBoardsEval.containsKey(used)) {
                    if(usedBoardsEval.get(used)> Math.abs(average)){
                        possibleReturnDuple = selectMoveHelper(2, used, depth + 1, usedBoardsAlpha.get(used), usedBoardsBeta.get(used));
